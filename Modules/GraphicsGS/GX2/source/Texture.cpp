/* Copyright (C) 2026 Kevin "MysticTortoise" Tessier */
#include "TeaPacket/Graphics/GS/Texture/Texture.h"
#include "../include/TeaPacket/Graphics/GS/PlatformTexture.hpp"

#include <cstdlib>

#include <gx2/mem.h>
#include <gx2/texture.h>
#include <gx2/utils.h>

#include "CafeGLSL/CafeGLSLCompiler.hpp"
#include "TeaPacket/Graphics/GS/GX2/GX2TextureFilter.gen"
#include "TeaPacket/Graphics/GS/GX2/GX2TextureFormat.gen"
#include "TeaPacket/Graphics/GS/GX2/GX2TextureWrap.gen"

#include "TeaPacket/Graphics/GS/Util/TextureFormatBits.h"

TP_GfxGS_Texture* TP_GfxGS_Texture_Create(const TP_GfxGS_TextureParams* params)
{
    auto* texture = new TP_GfxGS_Texture{
        .gx2Texture = GX2Texture{
            .surface = GX2Surface{
                .dim = GX2_SURFACE_DIM_TEXTURE_2D,
                .width = params->imageData.width,
                .height = params->imageData.height,
                .depth = 1,
                .mipLevels = 1,
                .format = TP_Gfx_Image_FormatToGX2(params->imageData.format),
                .aa = GX2_AA_MODE1X,
                .use = GX2_SURFACE_USE_TEXTURE,
                .imageSize = 0,
                .image = nullptr,
                .mipmapSize = 0,
                .mipmaps = nullptr,
                .tileMode = GX2_TILE_MODE_DEFAULT,
                .swizzle = 0,
                .alignment = 0,
                .pitch = 0,
                .mipLevelOffset = {}
            },
            .viewFirstMip = 0,
            .viewNumMips = 1,
            .viewFirstSlice = 0,
            .viewNumSlices = 1,
            .compMap = GX2_COMP_MAP(GX2_SQ_SEL_R, GX2_SQ_SEL_G, GX2_SQ_SEL_B, GX2_SQ_SEL_A),
            .regs = {}
        },
        .gx2Sampler = GX2Sampler{},
    };
    GX2CalcSurfaceSizeAndAlignment(&texture->gx2Texture.surface);
    GX2InitTextureRegs(&texture->gx2Texture);

    if (params->flags.shaderResource)
    {
        GX2InitSampler(
            &texture->gx2Sampler,
            TP_GfxGS_Texture_WrapModeToGX2(params->wrapMode),
            TP_GfxGS_Texture_FilterModeToGX2(params->filterMode));
    }

    texture->gx2Texture.surface.image = std::aligned_alloc(
        texture->gx2Texture.surface.alignment,
        texture->gx2Texture.surface.imageSize);

    if (params->imageData.data != nullptr)
    {
        GX2Surface proxySurface = texture->gx2Texture.surface;
        proxySurface.tileMode = GX2_TILE_MODE_LINEAR_SPECIAL;
        proxySurface.pitch = proxySurface.width;
        proxySurface.image = params->imageData.data;
        proxySurface.imageSize = static_cast<uint32_t>(TP_GfxGS_Helper_GetTexFormatBytesPerPixel(params->imageData.format) *
            static_cast<float>(params->imageData.width) *
            static_cast<float>(params->imageData.height));
        GX2CopySurface(&proxySurface, 0, 0, &texture->gx2Texture.surface, 0, 0);
    } else
    {
        memset(texture->gx2Texture.surface.image, 0, static_cast<int>(texture->gx2Texture.surface.alignment));
    }

    return texture;
}

void TP_GfxGS_Texture_SetActive(TP_GfxGS_Texture* texture, tp_u8 slot)
{
    GX2SetPixelTexture(&texture->gx2Texture, slot);
    GX2SetPixelSampler(&texture->gx2Sampler, slot);
}

tp_u16 TP_GfxGS_Texture_GetWidth(TP_GfxGS_Texture* const texture)
{
    return texture->gx2Texture.surface.width;
}

tp_u16 TP_GfxGS_Texture_GetHeight(TP_GfxGS_Texture* const texture)
{
    return texture->gx2Texture.surface.height;
}

TP_Gfx_Image_Format TP_GfxGS_Texture_GetFormat(TP_GfxGS_Texture* texture)
{
    return GX2ToTP_Gfx_Image_Format(texture->gx2Texture.surface.format);
}

void TP_GfxGS_Texture_Destroy(TP_GfxGS_Texture* texture)
{
    std::free(texture->gx2Texture.surface.image);
    delete texture;
}

tp_bool TP_GfxGS_Texture_IsFormatSupported(const TP_Gfx_Image_Format format)
{
    return TP_Gfx_Image_FormatConvertableToGX2(format);
}
