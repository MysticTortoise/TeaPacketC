...
static TP_GfxGS_Mesh* activeMesh;
...
TP_GfxGS_Mesh* TP_GfxGS_Mesh_Create(const TP_GfxGS_MeshParams* const params)
{
    ...
    for (size_t i = 0; i < sizeof(TP_GfxGS_MeshParams); i++)
    {
        TP_LogByte(ptr[i]);
    }
    ...
    for (size_t i = 0; i < params->vertexInfo.size; i++)
    {
        const size_t sizeOfElem = TP_GfxGS_ShaderVar_GetSize(params->vertexInfo.p[i]);
...
void TP_GfxGS_Mesh_SetActive(TP_GfxGS_Mesh* mesh)
{
