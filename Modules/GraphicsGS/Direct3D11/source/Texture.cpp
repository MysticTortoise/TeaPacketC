...
#include "TeaPacket/Graphics/GS/D3D11/D3D11TextureFilter.gen"
#include "TeaPacket/Graphics/GS/D3D11/D3D11TextureFormat.gen"
#include "TeaPacket/Graphics/GS/D3D11/D3D11TextureWrap.gen"
...
static constexpr D3D11_USAGE GetD3DUsage(const TP_GfxGS_TextureUseFlags flags)
{
    switch (flags.writeMode)
    {
    case TP_GfxGS_Texture_AvailableMode_None:
        return D3D11_USAGE_IMMUTABLE;
    case TP_GfxGS_Texture_AvailableMode_CPU:
        return D3D11_USAGE_DYNAMIC;
    case TP_GfxGS_Texture_AvailableMode_GPU:
...
static constexpr D3D11_BIND_FLAG GetD3DBindFlags(const TP_GfxGS_TextureUseFlags flags, const DTextureParms dParams)
{
    UINT flag = 0;
...
static constexpr D3D11_CPU_ACCESS_FLAG GetD3DCpuAccessFlags(const TP_GfxGS_TextureUseFlags flags)
{
    return static_cast<D3D11_CPU_ACCESS_FLAG>(
        flags.writeMode == TP_GfxGS_Texture_AvailableMode_CPU ? D3D11_CPU_ACCESS_WRITE : 0
        );
}

TP_GfxGS_Texture* D3D11::MakeTexture(const TP_GfxGS_TextureParams* params, DTextureParms dparams)
{
    assert(TP_GfxGS_Texture_IsFormatSupported(params->imageData.format));
    ...
    const size_t pitch = params->imageData.pitch != 0 ?
        ...
        static_cast<tp_u32>(std::ceil(static_cast<float>(params->imageData.width) *
            TP_GfxGS_Helper_GetTexFormatBytesPerPixel(params->imageData.format)));
    
    auto texture = new TP_GfxGS_Texture();
    ...
    textureDesc.Format = TP_Gfx_Image_FormatToD3D(texture->format);
    ...
    {
        assert(params->flags.writeMode != TP_GfxGS_Texture_AvailableMode_None);
    }
    ...
    if (params->flags.shaderResource)
    {
        ...
        samplerDesc.Filter = TP_GfxGS_Texture_FilterModeToD3D(params->filterMode);
        D3D11_TEXTURE_ADDRESS_MODE d3dWrapType = TP_GfxGS_Texture_WrapModeToD3D(params->wrapMode);
...
TP_GfxGS_Texture* TP_GfxGS_Texture_Create(const TP_GfxGS_TextureParams* params)
{
...
void TP_GfxGS_Texture_SetActive(TP_GfxGS_Texture* texture, tp_u8 slot)
{
...
tp_u16 TP_GfxGS_Texture_GetWidth(TP_GfxGS_Texture* texture)
{
    return texture->width;
}

tp_u16 TP_GfxGS_Texture_GetHeight(TP_GfxGS_Texture* texture)
{
    return texture->height;
}

TP_Gfx_Image_Format TP_GfxGS_Texture_GetFormat(TP_GfxGS_Texture* texture)
{
    return texture->format;
}

tp_bool TP_GfxGS_Texture_IsFormatSupported(const TP_Gfx_Image_Format format)
{
    // ReSharper disable once CppDFAConstantConditions
    if (!TP_Gfx_Image_FormatConvertableToD3D(format))
        return tp_false;

    UINT formatSupport;
    const HRESULT result = device->CheckFormatSupport(TP_Gfx_Image_FormatToD3D(format), &formatSupport);
...
void TP_GfxGS_Texture_Destroy(TP_GfxGS_Texture* texture)
{
    delete texture;
}
